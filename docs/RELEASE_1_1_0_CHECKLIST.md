# v1.1.0 Project Expansion Release Checklist

> 🛒 Official publication: **https://ramsandesh.gumroad.com**

## Version and compatibility

- [ ] `pyproject.toml` version is `1.1.0`.
- [ ] `umai.__version__` is `1.1.0`.
- [ ] `api/public_api_1_0.json` records `1.1.0` with the same stable 1.x symbol set.
- [ ] `CITATION.cff` records `1.1.0`.
- [ ] README version badge is `1.1.0`.
- [ ] `CHANGELOG.md` contains a `1.1.0` section.

## Project suite

- [x] 25 expected project entry points are tracked.
- [x] Every project default run emits valid JSON.
- [x] Five integrated capstones are included.
- [x] Five capstone subset snapshot fixtures are included.
- [x] Focused unit tests cover the five capstones.
- [x] `scripts/check_projects.py` validates exact inventory.
- [x] `scripts/check_project_snapshots.py` validates stable fixture subsets.

## Cross-platform quality

- [x] Dedicated project matrix covers Linux, Windows, and macOS.
- [x] Multiple supported Python versions are included in the project matrix.
- [x] Quality workflow runs project inventory and snapshot validation.
- [x] Existing package/publication/social-link quality gates remain enabled.

## Release verification

- [ ] Full Quality workflow passes on the exact 1.1.0 release commit.
- [ ] CI workflow passes on the exact 1.1.0 release commit.
- [ ] Project Matrix workflow passes on the exact 1.1.0 release commit.
- [ ] Create immutable `v1.1.0` from the verified commit.
- [ ] Publish GitHub release using `docs/COMPANION_RELEASE_1.1.0.md`.
- [ ] Build wheel/source/checksum assets from the immutable tag.
- [ ] Verify commercial eBook/publication files are not attached to the public software release.

## Administrative settings

Repository/account settings remain tracked separately, including `main` protection, prospective signing, repository homepage/topics, and secret-scanning options.

## Official publication

### **https://ramsandesh.gumroad.com**
