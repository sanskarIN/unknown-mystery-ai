...
## 2026-08-18 — Stable release automation and final publication hardening

### Version-aware stable publication

- Replaced the historical `v1.0.1`-specific stable publication workflow with a version-aware `Publish Stable` workflow.
- The workflow derives `v<version>` from `pyproject.toml` instead of hard-coding a previous release number.
- Stable publication requires the successful Quality run SHA to equal the current `main` SHA.
- Before creating a release, the workflow requires exact-commit success from:
  - CI
  - Quality
  - Project Matrix
  - Documentation Links
  - Release Check
- Missing or pending required runs make publication wait; failed required runs block publication.
- Matching versioned release notes, release checklist, and changelog section are required before publication.

### Immutable release assets

- Reworked `release-assets.yml` so software artifacts are built from the immutable published tag rather than a historical hard-coded tag or post-release `main`.
- The workflow verifies `tag == v<package version>`.
- It reruns repository/package/API/project/boundary validation before building release software.
- It builds and uploads:
  - Python wheel
  - source distribution
  - `SHA256SUMS.txt`
- Asset attachment is chained from successful `Publish Stable` workflow completion and also supports release/manual triggers.
- The workflow-run hand-off avoids depending only on a `release` event generated by `GITHUB_TOKEN`.

### Release automation regression protection

- Added `scripts/check_release_automation.py`.
- The checker validates the durable stable-publication and immutable-asset workflow contracts.
- It rejects stale hard-coded `v1.0.0` / `v1.0.1` release references in the current publication workflows.
- It verifies exact-commit workflow requirements, dynamic tag resolution, immutable-tag asset rebuilding, and publication-to-assets workflow chaining.
- Integrated the release-automation check into Quality, Release Check, and the manual Release Candidate workflow.
- Added `make release-automation` and aggregate `make verify` targets.

### Quality and workflow coverage

- Repository completeness now requires all durable CI/release workflows and the core quality/release scripts.
- Release Check now validates repository completeness, release documentation, action pins, stable release automation, release-candidate invariants, tests, projects, capstone snapshots, and built distributions.
- Project Matrix is triggered by stable release workflow changes so release-only edits still receive Linux/Windows/macOS project validation.
- The manual Release Candidate workflow now runs the expanded repository, release automation, policy, API, tests/examples/projects/snapshots, build, distribution, and checksum stack.

### Documentation

- Expanded the root `README.md` with stable-publication and immutable-asset behavior plus local release-automation validation commands.
- Expanded `docs/RELEASE_ASSETS.md` with exact-commit publication gates and immutable-tag provenance.
- Expanded `docs/RELEASE_RUNBOOK.md` with automated publication, workflow-run hand-off, asset verification, and post-release recovery rules.
- Expanded `docs/TESTING_STRATEGY.md` with release-automation, exact-commit publication, and immutable-tag validation layers.
- Expanded `docs/RELEASE_1_1_0_CHECKLIST.md` with stable-publication and asset-provenance requirements.
- Expanded `docs/COMPANION_RELEASE_1.1.0.md` and `CHANGELOG.md` to describe the final release hardening.

### Compatibility and publication boundary

- No incompatible `umai` 1.x public API change was introduced.
- No mandatory third-party runtime dependency was added.
- All 25 project identities and five capstone snapshot contracts remain intact.
- Paid eBook/manuscript/cover/certificate/customer publication assets remain excluded from the Apache-2.0 public software repository and GitHub release.
- Durable project/publication files continue to avoid a change-prone X/Twitter profile URL by default.
...
Continue with backward-compatible 1.x maintenance, regression fixes, optional project additions, accessibility improvements, and security/dependency/workflow updates after the verified 1.1.0 stable release.
