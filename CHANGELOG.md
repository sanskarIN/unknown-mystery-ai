...
- `make project-catalog`, `make projects`, `make project-snapshots`, `make release-check`, `make repository-check`, `make release-automation`, and `make verify` convenience targets
...
- `docs/RELEASE_RUNBOOK.md` with an exact maintainer procedure for release preparation, verification, publication automation, immutable-tag assets, post-release review, and patch recovery
- `scripts/check_repository_completeness.py` to validate required repository structure, documentation baseline, workflows, quality/release scripts, project-catalog count, and durable links
- `scripts/check_release_automation.py` to prevent stale hard-coded release tags and validate exact-commit publication plus immutable-tag asset workflow contracts

### Changed

- CI, Quality, Project Matrix, Documentation Links, Example Smoke Tests, and Release Check workflows cancel superseded runs with workflow/ref-or-PR concurrency groups
- High-frequency workflow jobs use explicit timeout limits to prevent indefinitely hanging jobs
- Quality validates repository/documentation completeness before metadata, policy, API, tests, project, build, and distribution checks
- Quality validates stable publication automation, project catalog integrity, all 25 companion projects, capstone snapshot fixtures, and 1.1.0 release-candidate invariants
- Project Matrix validates repository completeness and project catalog before project smoke/snapshot checks and is triggered by stable release workflow changes
- Release Check validates repository completeness, release documentation, full-SHA action pins, stable release automation, tests, projects, snapshots, and distribution contents
- Release Candidate manual workflow now runs the expanded repository, release-automation, policy, API, project, snapshot, build, and checksum stack
- Stable publication derives `v<version>` from package metadata instead of retaining a hard-coded historical version
- Stable publication requires the successful Quality SHA to equal current `main` and waits for exact-SHA success from CI, Quality, Project Matrix, Documentation Links, and Release Check
- Stable publication requires matching versioned release notes, versioned checklist, and changelog section before creating the tag/release
- Release-asset automation resolves the immutable published tag, validates tag/package identity, reruns repository/package/API/project/boundary checks, and builds wheel/source/checksum assets from the tag
- Release assets are chained from stable-publication workflow completion so asset attachment does not depend only on a release event generated by `GITHUB_TOKEN`
- Repository completeness now requires the full stable workflow set and core quality/release scripts
- Project smoke inventory expanded from twenty to twenty-five runnable projects
- Root README is a clearer repository hub with validation commands, all 25 projects, complete documentation, release automation, quality gates, limitations, licensing boundaries, and stable durable links
- `docs/TESTING_STRATEGY.md` documents repository completeness, release automation, API, examples, catalog, project, snapshot, cross-platform, documentation/policy, package, exact-commit, and immutable-tag validation layers
- `docs/RELEASE_ASSETS.md`, `docs/RELEASE_RUNBOOK.md`, and `docs/RELEASE_1_1_0_CHECKLIST.md` document the final stable publication and asset-provenance process
